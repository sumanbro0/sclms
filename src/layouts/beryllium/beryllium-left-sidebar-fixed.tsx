...
import { useWindowSize } from "packages/isomorphic-core/src/hooks/use-window-size";
import { userRoleAtom } from "@/cognito/login";
...
function MenuItems() {
  ...
  let menuItems: MenuItemsType[] = berylliumMenuItems;
  if (userRole !== "teacher") {
    console.log("isTeacher");
    menuItems = berylliumMenuItems.filter((item) => item.id !== "6");
  }
    ...
    <menu className="flex w-full justify-center">
      <SimpleBar className="h-[calc(100vh_-_105px)] w-full pb-5">
        <ul className="flex flex-col gap-6">
          {menuItems
            // .filter((item) => item.id === "5" && userRole === "teacher")
            .map((menu) => (
